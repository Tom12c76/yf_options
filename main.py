...
def plot_payoff():
    ...
    fig = make_subplots(rows=2, cols=2, shared_xaxes=True, shared_yaxes=True,
                        ...
                        column_titles=[f'<b>{long_short} {lots} lots of <br>{exp_date}  {ticker}  {strike}  strike  {strategy}s  @  {str(lastPrice)}', f'<b>Premium Paid = {lots * mult * lastPrice:,.0f}'],
    ...
    for i, w, o in zip(irange, width, opacity):
        ...
        fig.add_trace(go.Scatter(
            ...
            y=[(strike + i * pcf * lastPrice) / df_hist[ticker].iloc[0] - 1,
               (strike + i * pcf * lastPrice) / df_hist[ticker].iloc[0] - 1],
...
st.sidebar.write(f'Stock summary on [yahoo!] (https://finance.yahoo.com/quote/{ticker})')
